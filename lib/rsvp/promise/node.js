...
/**
  `RSVP.Promise.denodeify` takes a 'node-style' function and returns a function that
  ...
  ```javascript
  let fs = require('fs');
  let readFile = RSVP.Promise.denodeify(fs.readFile);
  ...
  ```javascript
  let request = RSVP.Promise.denodeify(require('request'));
  ...
  ```javascript
  let request = RSVP.Promise.denodeify(require('request'), true);
  ...
  ```javascript
  let request = RSVP.Promise.denodeify(require('request'), ['res', 'body']);
  ...
  let render = RSVP.Promise.denodeify(app.render.bind(app));
  ```
  ...
  let fs = require('fs');
  let readFile = RSVP.Promise.denodeify(fs.readFile);
  let writeFile = RSVP.Promise.denodeify(fs.writeFile);
